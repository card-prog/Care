...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "add") {
        $social = $_POST['social'] ?? '';
        if (!empty($social)) { // Fixed: Correct variable name
            $stmt = $conn->prepare("INSERT INTO social (social) VALUES (?)");
        ...
        $id = $_POST['id'] ?? '';
        $social = $_POST['social'] ?? '';
        if (!empty($id) && !empty($social)) {
            $stmt = $conn->prepare("UPDATE social SET social=? WHERE id=?");
            $stmt->bind_param("si", $social, $id);
            $stmt->execute();
            $stmt->close();
        }
...
$socialQuery = $conn->query("SELECT * FROM social");
?>
...
<div class="container mt-5">
  <div class="card p-4">
    <div class="profile-header">Social and Emotinal</div>

    

    <!-- Add social Form and Dropdown in a Row -->
    <form id="addsocialForm" class="row g-3 mt-3">
    <!-- Input for New social -->
    <div class="col-md-6">
        <label for="socialName" class="form-label">Add New social</label>
        <input type="text" class="form-control" id="socialName" placeholder="Enter new social..." required>
    </div>
    ...
    <!-- social Dropdown -->
    <div class="col-md-4">
        <label for="socialSelect" class="form-label">Select Social</label>
        <select id="socialSelect" class="form-select">
        <option value="">-- Select social --</option>
        <?php while ($row = $socialQuery->fetch_assoc()): ?>
            <option value="<?= htmlspecialchars($row['id']) ?>">
            <?= htmlspecialchars($row['social']) ?>
            </option>
        <?php endwhile; ?>
        </select>
    </div>
        ...
        <div class="table-wrapper">
            <table class="table table-hover border" id="dataTable">
                ...
                <tr>
                    ...
                    <th style="width: 60%;">Social</th>
                    <th style="width: 30%;">Actions</th>
                </tr>
                </thead>
                <tbody id="socialList">
                <?php
                $socialQuery->data_seek(0);
                while ($row = $socialQuery->fetch_assoc()): ?>
                    ...
                    <td>
                        <!-- Edit Button -->
                        <button class="icon-btn btn-edit editsocial"
                                data-id="<?= htmlspecialchars($row['id']) ?>" 
                                data-name="<?= htmlspecialchars($row['social']) ?>">
                        <i class="fas fa-edit"></i>
                        </button>

                        <!-- Delete Button -->
                        <button class="icon-btn btn-delete deletesocial" 
                                data-id="<?= htmlspecialchars($row['id']) ?>">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="modal-title-custom">Edit Social</h5>
            ...
            <div class="modal-body">
                <form id="editsocialForm">
                    ...
                    <div class="mb-4">
                        <label for="editsocialName" class="form-label fw-bold text-purple">Social Name:</label>
                        <input type="text" class="form-control input-underline" id="editsocialName" required>
                    </div>
...
$(document).ready(function() {
    // Add social
    $("#addsocialForm").submit(function(e) {
        e.preventDefault();
        let socialName = $("#socialName").val(); // Fixed variable
        $.post("", { action: "add", social: socialName }, function() {
            location.reload();
        });
    });
    ...
    // Edit social (Show Modal)
    $(".editsocial").click(function() {
        $("#editsocialId").val($(this).data("id"));
        $("#editsocialName").val($(this).data("name"));
        $("#editsocialModal").modal("show");
    });

    // Update social
    $("#editsocialForm").submit(function(e) {
        e.preventDefault();
        let id = $("#editsocialId").val();
        let socialName = $("#editsocialName").val(); // Fixed variable
        $.post("", { action: "update", id: id, social: socialName }, function() {
            location.reload();
        });
    });
});
