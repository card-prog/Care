...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "add") {
        $source = $_POST['source'] ?? '';
        if (!empty($source)) { // Fixed: Correct variable name
            $stmt = $conn->prepare("INSERT INTO source (source) VALUES (?)");
        ...
        if (!empty($id) && !empty($source)) {
            $stmt = $conn->prepare("UPDATE source SET source=? WHERE id=?");
...
$sourceQuery = $conn->query("SELECT * FROM source");
?>
...
<div class="container mt-5">
  <div class="card p-4">
    <div class="profile-header">Source of source and Assistant</div>
...
$(document).ready(function() {
    ...
    $("#addsourceForm").submit(function(e) {
        e.preventDefault();
        let sourceName = $("#sourceName").val(); // Fixed variable
        $.post("", { action: "add", source: sourceName }, function() {
            location.reload();
        });
    });
    ...
    $("#editsourceForm").submit(function(e) {
        e.preventDefault();
        let id = $("#editsourceId").val();
        let sourceName = $("#editsourceName").val(); // Fixed variable
        $.post("", { action: "update", id: id, source: sourceName }, function() {
            location.reload();
        });
    });
});
