...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "add") {
        $area = $_POST['area'] ?? '';
        if (!empty($area)) { // Fixed: Correct variable name
        ...
        if (!empty($id) && !empty($area)) {
            $stmt = $conn->prepare("UPDATE area SET area=? WHERE id=?");
...
// Get all area records
$areaQuery = $conn->query("SELECT * FROM area");
?>
...
<div class="container mt-5">
  <div class="card p-4">
    <div class="profile-header">Area of Difficulties</div>

    

    <!-- Add area Form and Dropdown in a Row -->
    <form id="addareaForm" class="row g-3 mt-3">
    <!-- Input for New area -->
    <div class="col-md-6">
        <label for="areaName" class="form-label">Add New Area</label>
        <input type="text" class="form-control" id="areaName" placeholder="Enter new area..." required>
    </div>
    ...
    <!-- area Dropdown -->
    <div class="col-md-4">
        <label for="areaSelect" class="form-label">Select Area of Difficulties</label>
        <select id="areaSelect" class="form-select">
        <option value="">-- Select Area of Difficulties --</option>
        <?php while ($row = $areaQuery->fetch_assoc()): ?>
        ...
        <div class="table-wrapper">
            <table class="table table-hover border" id="dataTable">
                ...
                <tr>
                    ...
                    <th style="width: 60%;">Area of Difficulties</th>
                    <th style="width: 30%;">Actions</th>
                </tr>
                </thead>
                <tbody id="areaList">
                <?php
                $areaQuery->data_seek(0);
                while ($row = $areaQuery->fetch_assoc()): ?>
                    ...
                    <td>
                        <!-- Edit Button -->
                        <button class="icon-btn btn-edit editarea"
                                data-id="<?= htmlspecialchars($row['id']) ?>" 
                                data-name="<?= htmlspecialchars($row['area']) ?>">
                        <i class="fas fa-edit"></i>
                        </button>

                        <!-- Delete Button -->
                        <button class="icon-btn btn-delete deletearea" 
                                data-id="<?= htmlspecialchars($row['id']) ?>">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="modal-title-custom">Edit Area</h5>
            ...
            <div class="modal-body">
                <form id="editareaForm">
                    ...
                    <div class="mb-4">
                        <label for="editareaName" class="form-label fw-bold text-purple">Area Name:</label>
                        <input type="text" class="form-control input-underline" id="editareaName" required>
                    </div>
...
$(document).ready(function() {
    ...
    $("#addareaForm").submit(function(e) {
        e.preventDefault();
        let areaName = $("#areaName").val(); // Fixed variable
        $.post("", { action: "add", area: areaName }, function() {
            location.reload();
        });
    });
    ...
    $("#editareaForm").submit(function(e) {
        e.preventDefault();
        let id = $("#editareaId").val();
        let areaName = $("#editareaName").val(); // Fixed variable
        $.post("", { action: "update", id: id, area: areaName }, function() {
            location.reload();
        });
    });
});
